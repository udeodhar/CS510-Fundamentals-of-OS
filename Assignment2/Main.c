...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  -- reading the output vertically, you can also see what each thread does.p
  --
  const
    BUFFER_SIZE = 5
  ...
  var
    ...
    full_buffs: Semaphore = new Semaphore
    empty_buffs: Semaphore = new Semaphore
    lockIn: Mutex = new Mutex                            -- Lock to protect bufferNextIn
    lockOut: Mutex = new Mutex                           -- Lock to protect bufferNextOut
    lockPrint: Mutex = new Mutex                         -- Lock to protect the core functioning
  
  function ProducerConsumer ()
      
      --Initializing semaphores
      full_buffs.Init(0)
      empty_buffs.Init(BUFFER_SIZE)                      
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        empty_buffs.Down()                             -- If the buffer is full, sleep.       

        lockIn.Lock()                                  -- Protect the in buffer.
	lockPrint.Lock()                               -- Protect the core functionality
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
        full_buffs.Down()                            -- If the buffer is empty, sleep.

	lockOut.Lock()                               -- Protect out buffer.
	lockPrint.Lock()                             -- Protect the core functionality.
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    superclass Object
    fields   
       condition: array [5] of Condition    -- For each fork (eating utensil)
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      status = new array of int {5 of THINKING}     -- All philosophers are thinking initially.
      
      condition = new array of Condition {5 of new Condition}
    ...
    method PickupForks (p: int)
	    mut.Lock()                       -- Acquire a lock before anything so that only one 
                                             -- thread is running in this section.
	    status[p] = HUNGRY               -- Mark philosopher 'p' hungry
            self.PrintAllStatus()

            self.Test(p)

	    while (status[p] != EATING)      -- Wait using MESA semantics.
		    condition[p].Wait(&mut)
	    endWhile
            mut.Unlock()                     -- Unlock the mutex
    endMethod

    method Test (p: int)
    -- Check whether your left neghbour or right neighbour is eating.
    -- Check whether you are hungry.
    -- If all is true then simply mark the philosopher 'p' eating.
    -- And finally signal the thread.
            if ((status[(p-1)%5] != EATING) && (status[(p+1)%5] != EATING) && (status[p] == HUNGRY))
                    status[p] = EATING
                    self.PrintAllStatus()
                    condition[p].Signal(&mut)
	    endIf
    
    endMethod

    method PutDownForks (p: int)
         mut.Lock()                           -- Lock the mutex so that no one else can run in this area.
         status[p] = THINKING                 -- Mark philosopher 'p' thinking
         self.PrintAllStatus()

         -- Check whether your right neighbour or left neighbour can eat.
         self.Test((p-1)%5)                   
         self.Test((p+1)%5)
