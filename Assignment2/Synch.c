...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
          waitingThreads = new List [Thread]                 -- Initializing the waiting thread list
        endMethod
      ...
      method Lock ()
          var oldIntStat: int
          oldIntStat = SetInterruptsTo (DISABLED)            -- Disable the interupts to avoid the race condition.

	  if (heldBy == currentThread)                       -- If the lock is alaready held by the current thread
                                                             -- then it should give the fatal error.
		  FatalError ("This thread already has the mutex and cannot ask for it again.")
	  endIf
	  if(!heldBy)                                        -- If heldBy is null then simply assig it to currentThread
		  heldBy = currentThread  
	  else                                               -- If it is held by some other thread, add this thread 
                                                             -- to the waiting list of this lock and sleep.
              waitingThreads.AddToEnd (currentThread)
              currentThread.Sleep()
          endIf
      ...
      method Unlock ()
           var t : ptr to Thread
               oldIntStat: int
           oldIntStat = SetInterruptsTo (DISABLED)
           if (heldBy != currentThread)                       -- Give fatal error if the lock is held by some other thread.
               FatalError ("Cannot unlock the Mutex which you don't have")
           endIf                      
           t = waitingThreads.Remove ()                       -- Remove the head of the queue.
           if (t)                                             -- If the queue is not null add it to the ready list.
               t.status = READY
               readyList.AddToEnd (t)                         
               heldBy = t                                     -- Set held by equal to this new thread. Because now this
                                                              -- thread holds the lock.
           else 
               heldBy = null                                  -- Set null if the waiting list is empty
           endIf
